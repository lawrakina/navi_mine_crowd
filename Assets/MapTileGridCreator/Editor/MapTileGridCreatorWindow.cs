...
/// Main window class.
/// </summary>
[CanEditMultipleObjects] public class MapTileGridCreatorWindow : EditorWindow {
    ...
    [SerializeField] private bool _debug_grid = true;
    [SerializeField] private int _size_grid = 20;
    [SerializeField] private int _offset_grid_y = 0;
    ...
    [SerializeField] [Min(1)] private float _dist_default_interaction = 100.0f;
    [SerializeField] private bool _collide_with_plane = true;
    [SerializeField] private string _path_pallet = "Assets/MapTileGridCreator/Pallets/Cubes";
    ...
    [SerializeField] private bool _debug_start_modifiers;
    private bool _showCreateMenu;
    private bool _showMapMenu;
    private bool _showTilePaletteMenu;
    private GUIStyle _styleExitButton;
    private GUIStyle _stileOn;
    private GUIStyle _stileOff;

    [MenuItem("MapTileGridCreator/Open")] public static void OpenWindows() {
    ...
    private void OnEnable() {
        _modes_paint = new GUIContent[]{
            ...
            new GUIContent(EditorGUIUtility.IconContent("Grid.PickingTool",
                                                        "Eyedropper to auto select the corresponding prefab in pallete"))
        };

        _modes_select = new GUIContent[]{
            new GUIContent(
                EditorGUIUtility.IconContent("Grid.Default", "Default, select or or multiple cells in scene view")),
            new GUIContent(EditorGUIUtility.IconContent("Grid.MoveTool", "Move selected cells to a given destination")),
            new GUIContent(EditorGUIUtility.IconContent("TreeEditor.Duplicate",
                                                        "Copy selected and paste to a given destination"))
        };
    }
    ...
    private void UpdateGridSelected(Grid3D selectedGrid) {
        if (selectedGrid != null)
        {
            ...
            {
                if (EditorUtility.DisplayDialog("Modify Existing Grid Prefab",
                                                "The grid selected is a prefab and cannot be modified unless you unpack it. " +
                                                "\n Do you want to continue ?", "Yes", "No"))
                {
                    PrefabUtility.UnpackPrefabInstance(selectedGrid.gameObject, PrefabUnpackMode.OutermostRoot,
    ...
    private Vector3 GetGridPositionInput(float offset_normal_factor, bool canCollideWithPlane) {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, _dist_default_interaction * (_grid.SizeCell + 1)) &&
            (!isPlaneCollided || (isPlaneCollided && hit.distance < enter)))
        {
            hitPoint = hit.point;
    ...
    private void CreateStyles() {
        _stileOn =  new GUIStyle(GUI.skin.button){
            normal = {textColor = Color.green},
            hover = {textColor = Color.red}
        };
        _stileOff = new GUIStyle(GUI.skin.button){
            normal = {textColor = Color.red},
            hover = {textColor = Color.green}
        };

        _styleExitButton = new GUIStyle(GUI.skin.button)
        {
            normal = {textColor = Color.magenta},
            active = {textColor = Color.yellow},
            focused = {textColor = Color.magenta}
        };
    }
    ...
    private void DrawMainMenu() {
        ...
        if (GUILayout.Button($"{(_showCreateMenu?"Hide":"Show")} Create Menu", _showCreateMenu?_stileOn:_stileOff))
        {
            _showCreateMenu = !_showCreateMenu;
        }

        if (GUILayout.Button($"{(_showMapMenu?"Hide":"Show")} selection Menu", _showMapMenu?_stileOn:_stileOff))
        {
            _showMapMenu = !_showMapMenu;
        }

        if (GUILayout.Button($"{(_showTilePaletteMenu?"Hide":"Show")} Tile palette Menu", _showTilePaletteMenu?_stileOn:_stileOff))
        {
            _showTilePaletteMenu = !_showTilePaletteMenu;
        }
        
        if(GUILayout.Button($"CLOSE", _styleExitButton))
        {
            _showCreateMenu = false;
            _showMapMenu = false;
            _showTilePaletteMenu = false;
            _mode_edit = EditMode.Select;
            _mode_select = SelectMode.Default;
        }
        ...
        if (_showMapMenu)
        {
            DrawMapSelectionPanel();
        }

        if (_showTilePaletteMenu)
        {
            DrawTilePalettePanel();
        }
    }

    private void DrawMapSelectionPanel() {
    ...
    private void DrawTilePalettePanel() {
        if (_grid != null)
        {
            ...
            _dist_default_interaction =
                EditorGUILayout.Slider("Distance default interaction : ", _dist_default_interaction, 0.0f, 500.0f);
            DrawEditor();
        }
    }
    ...
    private void DrawNewGridPanel() {
        ...
        _empty_creation_choice =
            (TypeGrid3D) EditorGUILayout.Popup((int) _empty_creation_choice, _empty_creation_choice.GetTypesGrid());
                ...
                if (_selection.Count != 0 && GUILayout.Button("Save to prefab"))
                {
                    ...
                    if (fullpath != "")
                    {
                        ...
                        foreach (Cell c in _selection)
                        {
                            ...
                            GameObject child =
                                PrefabUtility.InstantiatePrefab(prefabChild, prefab.transform) as GameObject;
                            child.name = c.name;
